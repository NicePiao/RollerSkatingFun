...
public class AudioBrowserListAdapter extends BaseAdapter {
    ...
    public View getViewMedia(int position, View convertView, ViewGroup parent) {
        ...
        if (mItemType == ITEM_WITH_COVER) {
            ...
            if (cover == null) {
                cover = BitmapCache.GetFromResource(v, R.drawable.music_default_icon);
            }
            ...
            paramsCover =
              new RelativeLayout.LayoutParams(0, RelativeLayout.LayoutParams.WRAP_CONTENT);
        }
        ...
        holder.footer.setVisibility(
          isMediaItemAboveASeparator(position) ? View.GONE : View.VISIBLE);
    ...
    private boolean isMediaItemAboveASeparator(int position) {
        ...
        if (mItems.get(position).mIsSeparator) {
            throw new IllegalArgumentException(
              "Tested item must be a media item and not a separator.");
        }
